...
Measured 2026-09-20 after this spec's work; each floor is set just under its
measured value so it can only ratchet up (CON-COV-002).

| Scope                | Lines    | Branches | Floor set |
| -------------------- | -------- | -------- | --------- |
| `src/lib/**`         | 97.8     | 92.5     | 96 / 91   |
| `scripts/lib/**`     | 98.6     | 94.9     | 96 / 93   |
| `src/components/**`  | 90.5     | 89.3     | 88 / 87   |
| `scripts/sources/**` | 87.4     | 91.2     | 85 / 89   |
| **Global**           | **85.0** | **91.1** | 84 / 90   |

**Where this falls short, stated plainly.** The goal is 95% global. It is 85%.
The whole of the remaining gap is the top-level `scripts/` CLI shells that do
heavy network I/O — `build-climate`, `build-landcover`, `build-detect`,
`build-gazetteer`, `build-names`, `build-geonames` — which sit at 28–45%. They
are the next tranche, and the same `run(io)` seam applies to each. The floors
above make that a one-way ratchet in the meantime. Nothing further was excluded
to close the gap on paper.

Note the per-directory table and the glob can disagree: `src/components/**`
also matches `src/components/ui/**`, so the glob's function coverage (73%)
reads lower than the directory row (77%). The floor follows the glob, because
that is what enforces.
...
- `run(io)` for each of: chunk-data, build-sitemap, build-search-index, build-pages,
  build-landcover, check-links, validate-data — each asserting the five contracts in section B
  that apply to it.
