...
The deterministic GitHub steps are in `scripts/pr-loop.sh`; the judgement
...
3. **Open the PR and keep it open:**
   `scripts/pr-loop.sh open` → capture the PR number. Do NOT merge yet.
4. **Review loop** — repeat, up to 5 rounds:
   ...
   6. **Resolve the threads** you addressed: `scripts/pr-loop.sh resolve-all`.
   7. Loop back to re-review the new diff.
5. **Gate before merge.** Confirm `scripts/pr-loop.sh status` shows checks
   green and `unresolved review threads: 0`. Wait for CI if it's still running.
6. **Approve** (reviewer role): `scripts/pr-loop.sh approve`. GitHub forbids
   self-approval, so this is best-effort — the real gate is step 5.
7. **Merge:** `scripts/pr-loop.sh merge` (squash + delete branch).
...
- Never merge with red checks or unresolved threads. `pr-loop.sh merge`
